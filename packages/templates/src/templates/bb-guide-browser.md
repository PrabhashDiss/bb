...
- `create [--url <http(s)-url>] [--reveal] [--profile <profile-id>]`: create a tab with a separate automation profile. Defaults: hidden, about:blank, and a fresh random profile that keeps nothing. `--profile <profile-id>` instead reuses the persistent automation profile of that name, so a profile seeded once with `import-cookies --into automation:<profile-id>` stays signed in for every later tab and thread. Any thread naming that profile carries its live sessions. Keep one profile per set of sites you are willing to share.
